...
**行为**: `StatusMonitor.vue` 当前将内存与磁盘区域升级为卡片化监控视图：内存卡片展示总量、已用、缓存、空闲和环形占比，磁盘卡片展示设备名、文件系统类型、读写速率以及挂载点/大小/可用/已用率表格；CPU、Swap、网络速率和 `StatusCharts.vue` 的 CPU / 网络曲线继续保留，其中 CPU 型号行会在主型号文案下方追加一个次级 badge，直接显示后端推送的 `cpuCores`（如 `16 核`）。  
**结果**: 状态监控从“简单进度行”升级为“高信息密度卡片”，并直接承接后端新增的内存细分字段、磁盘元数据和 CPU 核心数，无需用户再登录服务器手动查询机器规格。
